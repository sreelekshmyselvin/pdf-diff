...
def compute_changes(pdf_fn_1, pdf_fn_2, top_margin=0, bottom_margin=100):
	...
	docs = [serialize_pdf(0, pdf_fn_1, top_margin, bottom_margin),
			serialize_pdf(1, pdf_fn_2, top_margin, bottom_margin)]
...
def serialize_pdf(i, fn, top_margin, bottom_margin):
	...
	for run in box_generator:
		...
		if len(boxes) and (run['y'] - boxes[-1]['y']) > 20:
			text.append("  ")
			textlength += 2
...
def process_hunks(hunks, boxes):
	...
	for op, oplen in hunks:
		if op == "=":
			...
			# insertion.
			idx2 = 1 - idx
			mark_difference(1, offsets[idx2] - 1, boxes[idx2], changes, 1)
			mark_difference(0, offsets[idx2] + 0, boxes[idx2], changes, 2)
